...
#python op_no_chronology.py   --input "/home/harold-coder/Downloads/categories compliant (1).xlsx"   --output "/home/harold-coder/Desktop/compliance/op_no_chronology_sorted.xlsx"



DEFAULT_INPUT = "/home/harold-coder/Downloads/categories compliant (1).xlsx"
DEFAULT_SHEET = "Compliant with OP"
...
def extract_op_no(text: object) -> str:
    ...
    if not raw:
        return ""
    # Capture OP token with optional OP. prefix, e.g.:
    # OP.218/051/2003/049/2673
    # 218/051/2016-063
    match = re.search(r"(?i)\b(?:OP\.)?\d+/\d+/[A-Z0-9][A-Z0-9/\-_.]*", raw)
    if match:
...
def extract_registration_year(op_no: str) -> int | None:
    if not op_no:
        return None
    parts = [p.strip() for p in str(op_no).split("/")]
    if len(parts) < 3:
        return None
...
def main():
    ...
    parser.add_argument(
        "--output",
        default="op_no_chronology_sorted.xlsx",
